{{ 
  config(
    materialized = "table",
    unique_key = ["hashtag", "month_date"],
    partition_by = {
      "field": "month_date",
      "data_type": "date",
      "granularity": "month"
    },
    cluster_by = ["hashtag", "total_tweets"]
  ) 
}}
...
WITH base_tweets AS (
    ...
    LEFT JOIN {{ ref('02_politically_engaged_users') }} AS pe
...
final AS (
    SELECT
        ...
        -- Politician vs non-politician split
        ...
        -- Office counts
        COUNT(DISTINCT CASE WHEN t.office = 'Bundestag' THEN t.tweet_id END) AS bundestag_tweets,
        COUNT(DISTINCT CASE WHEN t.office = 'Landtag' THEN t.tweet_id END) AS landtag_tweets,
        COUNT(DISTINCT CASE WHEN t.office = 'Bundestag' THEN t.user_id END) AS bundestag_users,
        COUNT(DISTINCT CASE WHEN t.office = 'Landtag' THEN t.user_id END) AS landtag_users,

        -- Gender counts
        COUNT(DISTINCT CASE WHEN t.gender = 'M' THEN t.tweet_id END) AS male_tweets,
        COUNT(DISTINCT CASE WHEN t.gender = 'F' THEN t.tweet_id END) AS female_tweets,
        COUNT(DISTINCT CASE WHEN t.gender = 'M' THEN t.user_id END) AS male_users,
        COUNT(DISTINCT CASE WHEN t.gender = 'F' THEN t.user_id END) AS female_users

    FROM base_tweets AS t
    LEFT JOIN {{ ref('hashtag_categories_aggregated') }} AS hc
        ON t.hashtag = LOWER(hc.hashtag)
    WHERE month_date BETWEEN '2020-01-01' AND '2021-12-31'
    GROUP BY 1, 2, 3, 4
)
